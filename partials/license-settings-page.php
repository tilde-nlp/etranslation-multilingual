            ...
            <table class="form-table">
                    ...
                    <tr valign="top">
                            <th scope="row" valign="top">
                                    <?php esc_html_e('License Key', 'etranslation-multilingual'); ?>
                                </th>
                            <td>
                                    <div>
                                        ...
                                        <?php if( $status !== false && $status == 'valid' ) {
                                            ...
                                            $button_value = __('Deactivate License', 'etranslation-multilingual' );

                                            if( empty( $details['invalid'] ) )
                                                echo '<span title="'. esc_html__( 'Active on this site', 'etranslation-multilingual' ) .'" class="trp-active-license dashicons dashicons-yes"></span>';
                                            else
                                                echo '<span title="'. esc_html__( 'Your license is invalid', 'etranslation-multilingual' ) .'" class="trp-invalid-license dashicons dashicons-warning"></span>';

                                        }
                                        else {
                                            $button_name =  'trp_edd_license_activate';
                                            $button_value = __('Activate License', 'etranslation-multilingual');
                                        }
                                        ?>
                                        <input type="submit" class="button-secondary" name="<?php echo esc_attr( $button_name ); ?>" value="<?php echo esc_attr( $button_value ); ?>"/>
                                    </div>
                                    <p class="description">
                                            <?php esc_html_e( 'Enter your license key.', 'etranslation-multilingual' ); ?>
                                    </p>
                            </td>
                    </tbody>
                </table>
