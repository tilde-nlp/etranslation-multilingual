 ...
 *
 */
class ETM_Search extends WP_Query{
     ...
     * @return mixed
     */
    public function etm_search_filter( $query ) {
        global $ETM_LANGUAGE;

        if ( $ETM_LANGUAGE !== $this->settings['default-language'] ) {
            if ( ( !is_admin() && $query->is_main_query() && $query->is_search() ) || apply_filters( 'etm_force_search', false ) ) {
    ...
    public function get_post_ids_containing_search_term($search_query, $query = null ){
        ...
        $etm_search_query = '';
        $dictionary_name = $this->etm_query->get_table_name( apply_filters( 'etm_change_search_dictionary_language', $ETM_LANGUAGE, $this, $query ) );
        $meta_table_name =  $this->etm_query->get_table_name_for_original_meta();

        if( $search_terms_count === 1 ){
            /**
             * for one search term we can find it directly in the translated column
             */
            $etm_search_query = $this->db->prepare( "SELECT meta_value FROM
                                        $dictionary_name
                                        INNER JOIN $meta_table_name ON $dictionary_name.original_id = $meta_table_name.original_id AND $meta_table_name.meta_key = '". $this->etm_query->get_meta_key_for_post_parent_id() ."'
             ...
             */
            $etm_search_query = "SELECT meta_value FROM 
        ...
        $search_result_ids = $this->db->get_results( $etm_search_query, OBJECT_K );
     ...
     * @return string
     */
    public function etm_search_query( $s ){
        global $ETM_LANGUAGE;
