...
class ETM_Url_Converter {
		...
		foreach ( $languages as $language ) {
			...
			if ( ! in_array( $language_hreflang, $hreflang_duplicates ) ) {
				if ( isset( $hreflang_duplicates_region_independent[ $language ] ) ) {
					escape_and_echo_html( $hreflang_duplicates_region_independent[ $language ] );
				}
			}
		}
	...
	public function replace_iso_2_with_iso_3_for_hreflang( $hreflang, $language = null ) {
		...
		$hreflang_iso_1 = apply_filters(
			'etm_add_hreflang_correct_iso_code',
			array(
				'bel' => 'be',
			)
		);
	...
	public function add_etm_language_lang_attribute() {
		global $ETM_LANGUAGE;
		echo '<template id="etm-language" data-etm-language="' . esc_attr( $ETM_LANGUAGE ) . '"></template>';
	}
	...
	public function get_url_for_language( $language = null, $url = null, $etm_link_is_processed = '#ETMLINKPROCESSED' ) {
		...
		if ( ! $url_obj->isSchemeless() && $url_obj->getScheme() != 'http' && $url_obj->getScheme() != 'https' ) {
			etm_bulk_debug(
				...
				array(
					'url'   => $url,
					'abort' => 'is different scheme ' . $url_obj->getScheme(),
				)
			);
			wp_cache_set( 'get_url_for_language_' . $hash, $url . $etm_link_is_processed, 'etm' );
		...
		if ( $url_obj->isSchemeless() && ! $url_obj->getPath() ) {
			etm_bulk_debug(
				...
				array(
					'url'   => $url,
					'abort' => 'is anchor or has get params',
				)
			);
		...
		if ( $this->get_lang_from_url_string( $url ) === null && $this->settings['default-language'] === $language && $this->settings['add-subdirectory-to-default-language'] !== 'yes' ) {
			etm_bulk_debug(
				...
				array(
					'url'   => $url,
					'abort' => 'URL already has the correct language added to it and default language has subdir',
				)
			);
			wp_cache_set( 'get_url_for_language_' . $hash, $url, 'etm' );
			return $url;
		...
		if ( $this->get_lang_from_url_string( $url ) === $language ) {
			etm_bulk_debug(
				...
				array(
					'url'   => $url,
					'abort' => 'URL already has the correct language added to it',
				)
			);
			wp_cache_set( 'get_url_for_language_' . $hash, $url, 'etm' );
			return $url;
		...
		if ( $possible_post_id ) {
			...
			etm_bulk_debug(
				...
				array(
					'url'           => $url,
					'found post id' => $post_id,
					'for language'  => $ETM_LANGUAGE,
				)
			);
		} else {
			...
			if ( $post_id ) {
				etm_bulk_debug(
					...
					array(
						'url'                  => $url,
						'found post id'        => $post_id,
						'for default language' => $ETM_LANGUAGE,
					)
				); }
		...
		$new_url_has_been_determined === false ) {// for some reason in yoast sitemap is_home() is true ..so we need to check if we are not in the sitemap itself
			$ETM_LANGUAGE = $language;
			if ( empty( $url_obj->getQuery() ) ) {
			...
			if ( $this->get_lang_from_url_string( $url ) === null ) {
				...
				etm_bulk_debug(
					...
					array(
						...
						'new url'  => $new_url,
						'lang'     => $language,
						'url type' => 'custom url without language parameter',
					)
				);
			} else {
				...
				etm_bulk_debug(
					...
					array(
						...
						'new url'       => $new_url,
						'lang'          => $language,
						'url type'      => 'custom url with language',
						'abs home path' => $abs_home_url_obj->getPath(),
					)
				);
	...
	public function cur_page_url() {
		...
		if ( function_exists( 'apply_filters' ) ) {
			$req_uri = apply_filters( 'etm_curpageurl', $req_uri );
		}
