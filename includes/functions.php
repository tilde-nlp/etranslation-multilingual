...
function etm_the_language_switcher() {
	...
	escape_and_echo_html( $language_switcher->language_switcher() );
}

function emt_safe_json_send( $value ) {
	...
	switch ( json_last_error() ) {
		...
		case JSON_ERROR_DEPTH:
			return wp_send_json( 'Maximum stack depth exceeded' );
		case JSON_ERROR_STATE_MISMATCH:
			return wp_send_json( 'Underflow or the modes mismatch' );
		case JSON_ERROR_CTRL_CHAR:
			return wp_send_json( 'Unexpected control character found' );
		case JSON_ERROR_SYNTAX:
			return wp_send_json( 'Syntax error, malformed JSON' );
		case JSON_ERROR_UTF8:
			return wp_send_json( etm_utf8ize( $value ) );
		default:
			return wp_send_json( 'Unknown error' );
	}
}
...
function escape_and_echo_html( $html ) {
	...
	$data_attributes           = array(
		'data-name'            => array(),
		'data-setting-name'    => array(),
		'data-column-name'     => array(),
		'data-confirm-message' => array(),
	);
	$allowed_tags['input']    += $data_attributes;
