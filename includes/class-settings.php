...
class ETM_Settings {
	...
	public function get_language_switcher_options() {
		$ls_options = apply_filters(
			'etm_language_switcher_output',
			array(
				'full-names'         => array(
					'full_names'  => true,
					'short_names' => false,
					'flags'       => false,
					'no_html'     => false,
					'label'       => __( 'Full Language Names', 'etranslation-multilingual' ),
				),
				'short-names'        => array(
					'full_names'  => false,
					'short_names' => true,
					'flags'       => false,
					'no_html'     => false,
					'label'       => __( 'Short Language Names', 'etranslation-multilingual' ),
				),
				'flags-full-names'   => array(
					'full_names'  => true,
					'short_names' => false,
					'flags'       => true,
					'no_html'     => false,
					'label'       => __( 'Flags with Full Language Names', 'etranslation-multilingual' ),
				),
				'flags-short-names'  => array(
					'full_names'  => false,
					'short_names' => true,
					'flags'       => true,
					'no_html'     => false,
					'label'       => __( 'Flags with Short Language Names', 'etranslation-multilingual' ),
				),
				'only-flags'         => array(
					'full_names'  => false,
					'short_names' => false,
					'flags'       => true,
					'no_html'     => false,
					'label'       => __( 'Only Flags', 'etranslation-multilingual' ),
				),
				'full-names-no-html' => array(
					'full_names'  => false,
					'short_names' => false,
					'flags'       => false,
					'no_html'     => true,
					'label'       => __( 'Full Language Names No HTML', 'etranslation-multilingual' ),
				),
			)
		);
		return $ls_options;
	...
	public function show_invalid_language_codes_error_notice() {
		...
		$error_manager->record_error(
			array(
				'message'         => esc_html__( 'Language codes can contain only A-Z a-z 0-9 - _ characters. Check your language codes in eTranslation Multilingual General Settings.', 'etranslation-multilingual' ),
	...
	public function get_default_etm_machine_translation_settings() {
		return apply_filters(
			'etm_get_default_etm_machine_translation_settings',
			array(
	...
	public function enqueue_scripts_and_styles( $hook ) {
		if ( in_array( $hook, array( 'settings_page_etranslation-multilingual', 'admin_page_etm_advanced_page', 'admin_page_etm_machine_translation', 'admin_page_etm_test_machine_api' ) ) ) {
			wp_enqueue_style(
				'etm-settings-style',
