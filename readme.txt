=== eTranslation Multilingual ===
Contributors: Tilde
Tags: translate, translation, etranslation, EU, europe, multilingual, automatic translation, bilingual, front-end translation, language
Requires at least: 3.1.0
Tested up to: 6.0.1
Requires PHP: 5.6.20
Stable tag: 1.0.0
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Make your site multilingual in few steps with eTranslation Multilingual Wordpress plugin. 

== Description ==

The eTranslation Multilingual WordPress plugin allows full website content translation in any language supported by the eTranslation automated translation service provided by the EC. The automated translations are performed by an eTranslation service API.


== Features ==

* Visual translation editor
* Automatic translation in 30 languages via eTranslation integration
* Easy setup
* Image translation support
* Translation caching for quick page load times
* Dynamic content translation
* Compatible with WooCommerce, Gutenberg and other plugins & themes
* Configurable language switcher

== Installation ==

1. Upload etranslation-multilingual.zip file from Wordpress dashboard -> Plugins -> Add new -> Upload Plugin -> Choose file -> Install Now
1.1.	Alternatively, upload unzipped etranslation-multilingual folder to the ‘/wp-content/plugins/’ directory
2.	Activate installed plugin through ‘Plugins’ menu (Plugins -> eTranslation Multilingual -> Activate)
3.	Configure website source & target languages, eTranslation credentials for automatic translation from plugin settings (Settings -> eTranslation Multilingual)


== Screenshots ==

1. eTranslation Multilingual language and domain configuration
2. Automatic translation configuration with eTranslation credentials
3. Floating Language Switcher
4. Selecting page language
5. Translated page, machine-translated content notice
6. Visual translation editor


== Changelog ==

= 1.0.0 =
* Initial release.
